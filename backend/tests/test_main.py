...
def test_get_audio_features_endpoint_bad_request_1(client):
    response = client.post("/audio-features")
    assert response.status_code == 400
    assert response.json["error"] == "content_type must be application/json"


def test_get_audio_features_endpoint_bad_request_2(client):
    response = client.post("/audio-features", data=json.dumps({}))
    assert response.status_code == 400
    assert response.json["error"] == "content_type must be application/json"
...
def test_get_audio_features_endpoint(client):
    response: Response = client.post("/audio-features", data=json.dumps({
        "data": [
            "0UaMYEvWZi0ZqiDOoHU3YI",
            "6I9VzXrHxO9rA9A5euc8Ak",
            "0WqIKmW4BTrj3eJFmnCKMv",
...
def test_recommend_playlist_tracks_endpoint_no_input(client):
    response = client.post("/recommend-playlist-tracks")
    assert response.status_code == 400
    assert response.json["error"] == "content_type must be application/json"


def test_recommend_playlist_tracks_endpoint_no_content_type(client):
    response = client.post("/recommend-playlist-tracks", data=json.dumps({}))
    assert response.status_code == 400
    assert response.json["error"] == "content_type must be application/json"


def test_recommend_playlist_tracks_endpoint_no_data_field(client):
    response = client.post(
        "/recommend-playlist-tracks", data=json.dumps({}), content_type='application/json')
    assert response.status_code == 400
    assert response.json["error"] == "request body must contain a data field"


def test_recommend_playlist_tracks_endpoint_empty_input(client):
    response = client.post("/recommend-playlist-tracks", data=json.dumps({
        "data": []
    }), content_type='application/json')
    assert response.status_code == 400
    assert response.json["error"] == "data must be a playlist_uri string"

# NOTE: broken
# def test_recommend_playlist_tracks_endpoint(client):
#     response = client.post("/recommend-playlist-tracks", data=json.dumps({
#         "data": [
#             "0UaMYEvWZi0ZqiDOoHU3YI",
#             "6I9VzXrHxO9rA9A5euc8Ak",
