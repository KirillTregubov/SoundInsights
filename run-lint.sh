...
source init-logging.sh run-lint
...
clean_up () {
  ...
  docker compose --file docker-compose-lint.yml down -v --rmi all --remove-orphans
  trap "" EXIT
  exit 0
}
...
docker compose --file docker-compose-lint.yml up --build